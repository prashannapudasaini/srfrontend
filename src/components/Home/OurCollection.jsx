...
const nepaliFontClass = isNepali ? "nepali-heading" : "tracking-wider";
  ...
  return (
    // 🔥 FIX: Kept your pb-12 for mobile, but added md:pb-20 to keep it breathing on desktop
<section className="pt-8 pb-12 md:pb-20 bg-[#FDF8E7] relative overflow-hidden">
  {/* Background Effects */}
  ...
  <div className="max-w-7xl mx-auto px-6 relative z-10">
    ...
    <motion.div
      ...
      viewport={{ once: true }}
      className="text-center mb-8 md:mb-16" /* 🔥 FIX: Reduced from mb-16 to mb-8 for mobile */
    >
      
      <h2 className={`text-[#9e111a] text-sm uppercase font-bold mb-2 md:mb-4 ${isNepali ? `${nepaliFontClass} inline-block py-1` : 'tracking-[0.3em]'}`}>
        {t('ourCollection.subtitle', 'Our Collection')}
      </h2>

      {/* 🔥 FIX: Added text-4xl for mobile so it fits nicely, and reduced mb-6 to mb-3 */}
      <h3 className={`text-4xl md:text-5xl lg:text-6xl font-serif font-extrabold text-[#1A1A1A] mb-3 md:mb-6 drop-shadow-sm ${isNepali ? `${nepaliFontClass} block pb-1 md:pb-2` : ''}`}>
        {t('ourCollection.titleLine1', 'Dairy ')} <span className="text-[#9e111a]">{t('ourCollection.titleLine2', 'Selection')}</span>
      </h3>

      <div className="w-16 md:w-24 h-1 bg-[#9e111a] mx-auto rounded-full" />
    </motion.div>
        ...
        >
          {[...products].reverse().map((product) => {
            ...
            const subtitle = product.variants?.[0]?.description || t('ourCollection.fallbackSubtitle', "Farm Fresh Dairy");
              ...
              >
                <div className="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-500 flex flex-col h-full z-10 relative">
                  ...
                  <div className="relative h-64 overflow-hidden bg-white flex items-center justify-center">
                    ...
                    {product.badge && (
                      <span className={`absolute top-4 right-4 bg-[#9e111a] text-white px-3 py-1 text-[10px] font-black rounded uppercase shadow-md ${isNepali ? nepaliFontClass : 'tracking-widest'}`}>
                        {product.badge}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-6 flex flex-col flex-grow text-center">
                    <p className={`text-[#9e111a] text-[10px] font-black uppercase mb-2 ${isNepali ? nepaliFontClass : 'tracking-widest'}`}>
                      {product.category}
                    </p>

                    <h4 className={`text-xl font-serif font-bold text-[#1A1A1A] mb-1 line-clamp-1 ${isNepali ? nepaliFontClass : ''}`}>
                      {product.name}
                    </h4>

                    <p className={`text-gray-500 font-medium text-xs mb-4 line-clamp-1 ${isNepali ? nepaliFontClass : ''}`}>
                      {subtitle}
                    </p>

                    <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                      <span className={`text-lg font-black text-[#1A1A1A] ${isNepali ? nepaliFontClass : ''}`}>
                        {t('common.currency', 'NPR')} {lowestPrice}<span className="text-xs text-gray-400 font-bold ml-1">{unit}</span>
                      </span>
