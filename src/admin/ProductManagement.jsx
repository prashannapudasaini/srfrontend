  ...
  const handleDeleteCategory = (id) => {
    showConfirm("Are you sure you want to permanently delete this category?", async () => {
      try {
        ...
        showAlert("Failed to delete category due to a server error."); 
      }
    });
  };
    ...
    showConfirm("Are you sure you want to permanently delete this product? This action cannot be undone.", async () => {
      try {
        ...
        if (res.data.status === 'success') {
          ...
          showAlert(res.data.message || "Failed to delete product.");
        }
      } catch (error) {
        console.error(error);
        showAlert("Error connecting to server to delete product.");
      }
    });
  };
  ...
  const handleSaveProduct = async (savedProduct) => {
    try {
      ...
      if (res.data.status === 'success') {
        ...
        showAlert(res.data.error || res.data.message || "Failed to save product.");
      }
    } catch (error) {
      console.error(error);
      showAlert("Error connecting to server while saving.");
    }
  };
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="bg-[#1A1A1A] p-6 lg:p-8 rounded-[2rem] lg:rounded-[3rem] flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-xl">
        ...
        <button onClick={openAddModal} className="bg-[#9e111a] w-full md:w-auto text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white hover:text-[#1A1A1A] transition-all flex items-center justify-center gap-2">
      ...
      <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-[#9e111a]/5 flex flex-col lg:flex-row gap-4 justify-between items-center">
        ...
        <div className="relative w-full lg:w-72 shrink-0">
          ...
          <input type="text" placeholder="Search products..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-gray-50 border border-gray-200 text-sm rounded-xl py-2.5 pl-10 pr-4 focus:outline-none focus:border-[#9e111a] transition-all" />
        </div>
      </div>
  ...
  const handleImageUpload = async (file, isMain = false, variantIndex = null) => {
    ...
    try {
      ...
      if (res.data.status === 'success') {
        ...
        showAlert("Image upload failed on the server.");
      }
    } catch (error) {
      showAlert("Error uploading image. Check your network.");
    } finally {
          ...
          <form id="productForm" onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="md:col-span-3 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-xs font-bold text-gray-500 mb-2">Category</label>
                      {categories.length === 0 ? <select disabled className="w-full p-3 rounded-xl border border-gray-200 text-gray-400 bg-gray-50 outline-none"><option>Create a category first</option></select> : <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full p-3 rounded-xl border border-gray-200 focus:border-[#9e111a] outline-none bg-white">{categories.map(c => <option key={c} value={c}>{c}</option>)}</select>}
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                        <label className="flex items-center gap-3 cursor-pointer group"><input type="checkbox" checked={formData.is_premium} onChange={e => setFormData({...formData, is_premium: e.target.checked})} className="w-4 h-4 text-[#9e111a] rounded cursor-pointer" /><span className="text-xs font-bold text-gray-700 cursor-pointer transition-colors group-hover:text-[#9e111a]">Show in "Premium Selection"</span></label>
                        <label className="flex items-center gap-3 cursor-pointer group"><input type="checkbox" checked={formData.is_essential} onChange={e => setFormData({...formData, is_essential: e.target.checked})} className="w-4 h-4 text-[#9e111a] rounded cursor-pointer" /><span className="text-xs font-bold text-gray-700 cursor-pointer transition-colors group-hover:text-[#9e111a]">Show in "Dairy Essentials"</span></label>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <button type="button" onClick={addVariant} className="bg-[#002147] text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-[#E2B254] hover:text-[#002147] transition-all"><PlusCircle size={14} /> Add Variant</button>
              </div>
